...
void wave_Consts_Test(double *rho2, double *X2, gsl_rng *rng, double lambda){
  ...
  *X2=0;
  ...
  for (int i=0; i<p; i++){
    a[i]=gsl_ran_gaussian(rng, sigma/sqrt(p))+mu;
    b[i]=gsl_ran_gaussian(rng, sigma/sqrt(p))+mu;
    asum+=a[i]; bsum+=b[i];
  }
...
int main(int argc, char *argv[]) {  
  ...
  gsl_rng *x2rng=gsl_rng_alloc(gsl_rng_mt19937);
  gsl_rng_set(x2rng, 600);
  
  double a;//=malloc(sizeof(double));
  double b;//=malloc(sizeof(double));
    
  for (int z=0; z<1000000; z++){
    wave_Consts_Test(&a, &b, x2rng, 5.0);
    printf("%f %f\n", a, b);
  }  
  ...
  #pragma omp parallel
  {
    ...
    //ATOMIC MOVE FROM threadTotals TO efficiency
    #pragma omp critical
    for(j=D_steps; j--;)
...
ssize_t str2network(LALDetector network[LAL_NUM_DETECTORS], double reach[LAL_NUM_DETECTORS], char *str) {
  ...
  while (k < LAL_NUM_DETECTORS && str[k]) {
    ...
    reach[k]=horizon_distance(1.4, 1.4, 10., 1570., 8., str[k]);
